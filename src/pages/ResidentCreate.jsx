    ...
    axios
      ...
      .then((response) => {
        ...
        if (response.status === 201) {
          toast.success('Added a new residential record') // SUCCESSFUL OPERATION
          navigate('/residents/' + response.data.id, { replace: true })
        }
      })
      .catch((error) => {
        setStatus('success')
  ...
  return (
    <Authorization permissions={ACCOUNT.permissions} permission="write_resident">
      <Form status={status}>
        <PageTitle
          title="New Resident"
          description="Create a new resident record. For unknown answers, please leave it blank. For date visited, January to June will be considered as first visit and July to December as second visit."
        />
